...
public class ProductFileController(IUserService userService, IProductFileService productFileService) : AuthorizedController(userService)
{
    ...
    public async Task<ActionResult<RequestResponse>> Add([FromForm] ProductImageAddDTO form) // The FromForm attribute will bind each field from the form request to the properties of the UserFileAddDTO parameter.
                                                                                         // For files the property should be IFormFile or IFormFileCollection.
    {
        var currentUser = await GetCurrentUser();

        return currentUser.Result != null ?
            FromServiceResponse(await productFileService.SaveImage(form, currentUser.Result)) :
            ErrorMessageResult(currentUser.Error);
    }

    /// <summary>
    /// This method downloads a product file
    /// </summary>
    [HttpGet("{id:guid}")]
    ...
    {
        var file = await productFileService.GetImageDownload(id);
